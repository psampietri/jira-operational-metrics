...
function App() {
  ...
  const handleProjectKeyChange = useCallback(async (key) => {
    ...
    try {
      ...
      const defaultGroups = meta.statuses.map((s) => ({
      ...
      setMetadataError(
        `Failed to load project '${key}'. Check project key and backend connection.`,
      );
    } finally {
  ...
  const handleStatusGroupsChange = (newGroups) => {
    ...
    // If we already have data, re-process it with the new groups
    if (issues.length > 0) {
      setIsLoading(true);
      try {
        const data = processMetrics(issues, newGroups);
        setProcessedData(data);
      } catch (err) {
        console.error('Error re-processing metrics:', err);
        setError('Failed to re-process data with new groups.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleFilterSubmit = useCallback(
    async (jqlFilter) => {
      ...
      try {
        // 1. Fetch all tickets from the backend (which handles pagination)
        ...
        if (rawIssues.length === 0) {
          setProcessedData(null); // No data to show
          return;
        }

        // 2. Process the raw issues using the *current* statusGroups
        const data = processMetrics(rawIssues, statusGroups);
        setProcessedData(data);
      } catch (err) {
        console.error('Failed to fetch or process tickets', err);
        setError('Failed to load ticket data. Check console for details.');
      } finally {
        setIsLoading(false);
      }
    },
    [projectKey, statusGroups], // Depends on projectKey and the current groups
  );
